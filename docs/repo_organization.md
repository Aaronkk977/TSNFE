...
- `data/processing/checkpoints/`：批次處理檢查點（預留，現階段很少使用）。
- `data/processing/errors/`：失敗記錄（下載/處理）。
- `data/processing/metadata/`：抓取清單與輔助 metadata。
- `data/processing/debug/`：除錯輸出（例如 LLM 原始回應）。
- `data/processed/signals/`：訊號輸出 JSON 與 recommendation list。
- `data/processed/reports/`：自動化報表輸出（矩陣表、每日摘要）。

## 3) 已完成的整理

- `failed_downloads.json` → `data/processing/errors/failed_downloads.json`
- `failed_processing.json` → `data/processing/errors/failed_processing.json`
- `video_list.json` → `data/processing/metadata/video_list.json`
- `logs/last_gemini_multimodal_response.json` → `data/processing/debug/last_gemini_multimodal_response.json`
- 根目錄零散檔案移至 `local/`：
